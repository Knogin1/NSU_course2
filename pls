...
def days_between(dstr1, dstr2):
    ...
    """
    #return abs(datatime.strptime(dstr1, "%Y-%m-%d") - datatime.strptime(dstr1, "%Y-%m-%d"))
    return abs((datetime.strptime(dstr1, "%Y-%m-%d") - datetime.strptime(dstr2, "%Y-%m-%d")).days)
...
def replacer(text, pattern):
    ...
    *** expressions are ***
    ...
    # \b границы слов, \w*{re.escape(pattern)}\w* — любое слово, содержащее pattern
    # re.sub() замена на ***
